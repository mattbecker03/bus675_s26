...
# Set this to your data directory (Google Drive path if using Colab)
DATA_DIR  = Path("../data/movie_posters")
IMAGE_DIR = DATA_DIR / "images"

CHECKPOINT_PATH = "best_model_partA.pth"
...
class MoviePosterDataset(Dataset):
    def __init__(self, df, image_dir, vocab_builder, numeric_scaler, transform=None):
        ...
        # Pre-compute scaled numeric features once — shape (N, len(NUMERIC_COLS))
    ...
    def __getitem__(self, idx):
        ...
        # Load and transform poster image
...
class ImageBranch(nn.Module):
    """
    Four conv blocks that progressively halve spatial size, then global
    average pool + linear projection to out_dim.
...
class TabularBranch(nn.Module):
    ...
    Two sub-branches merged into one output vector:
      - Numeric sub-branch: MLP over standardised numeric features
      - Embedding sub-branch: one embedding table per categorical field,
        mean-pooled over tokens, concatenated, then through a small MLP
    Output: (batch, out_dim)
    """
...
class FusionHead(nn.Module):
    """Concatenates image and tabular vectors, produces 6-class logits."""
